...
Lemma In_get:
  forall m b t
         (HWF:Ir.Memory.wf m)
         (HIN:List.In (b, t) (Ir.Memory.blocks m)),
    Some t = Ir.Memory.get m b.
Proof.
  intros.
  unfold Ir.Memory.get.
  assert (list_find_key (Ir.Memory.blocks m) b = [(b, t)]).
  ...
  { destruct l'. eauto.
    ...
    { destruct p. destruct p0.
      ...
      { rewrite PeanoNat.Nat.eqb_eq in HEQID.
        subst b0.
        assert (List.In (b, t) (Ir.Memory.blocks m)).
      ...
      { rewrite PeanoNat.Nat.eqb_neq in HEQID.
        ...
        { eapply disjoint_lsubseq_disjoint.
          ...
          assumption. }
        assert (disjoint_range (Ir.MemBlock.P0_range t)
                               (Ir.MemBlock.P0_range t0) = true).
